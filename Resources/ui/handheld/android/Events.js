			...
			if (status == '200') {
				...
				if (Json.results.length > 0) {
					if(that.data.menuItem.title == 'Nearby'){
						...
						getCurrentPosition(Json.results);
							//that.EventsWin.add(createScroll(that, Json.results));
					}else{
						var jsonArr =[];	
						for (var i = 0; i < Json.results.length; i++) {
							if((Json.results[i].venue[that.data.menuItem.category] == that.data.menuItem.title) || (Json.results[i][that.data.menuItem.category] == that.data.menuItem.title)){
