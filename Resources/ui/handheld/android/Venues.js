...
Venues.prototype.init = function(argument, isFlyout) {
	...
	Ti.App.addEventListener('geoPositionVenues', function(e){
		var jsonArr =[];	
		for (var i = 0; i < e.json.length; i++) {
			var lat = e.latitude,
				lng = e.longitude,
				glat = e.json[i].geolocation.latitude,
				glong = e.json[i].geolocation.longitude;
			//echo(lat);
		...
		if(jsonArr.length){
			that.VenuesWin.add(createScroll(that, jsonArr));
		}else{
			alert('No venues within '+ radius +' '+Ti.App.Properties.getString('units'));
		}
		
	});
	//this.VenuesWin.add(createScroll(that, globals.venues.json));
		...
		Parse.queryObjects('Venue', query, function(e, response, status) {
			if (status == '200') {
				...
				if (Json.results.length > 0) {	
					if(that.data.menuItem.title == 'Nearby'){
							getCurrentPosition(Json.results,'venues');
					}else{		
						var jsonArr =[];	
						for (var i = 0; i < Json.results.length; i++) {
							
							if(Json.results[i][that.data.menuItem.category] == that.data.menuItem.title){
